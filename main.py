...
def main2(f1,f2):
    # If args['hetero'] is True, g would be a heterogeneous graph.
    # Otherwise, it will be a list of homogeneous graphs.
    labels,graphs,features = dataloader(f1,f2)
    train_graphs = graphs[:100]
    val_graphs = graphs[100:130]
    test_graphs = graphs[130:]
    train_labels = labels[:100]
    val_graphs = labels[100:130]
    test_grapps = labels[130:]
    train_features = features[:100]
    val_features = labels[100:130]
    test_features = features[130:]

    model = HAN(meta_paths=[['CF', 'FW'],[ 'DF', 'FB']],
    ...
    for epoch in range(args['num_epochs']):
        ...
        for i,graph in enumerate(train_graphs):
            ...
            logits = model(graph)
            loss = loss_fcn(logits, train_labels[i])
...
def main3(f1,f2):
    labels,graphs = dataloader(f1,f2)
    data = zip(graphs,labels)
